...
def mobileNet(name, mptcpEnabled, fdmEnabled, congestCtl, replay, configFile):
    ...
    # net.startMobility(time=0, AC=acMode, model='RandomDirection', max_x=200, max_y=160, min_x=0, min_y=30, max_v=0.8, min_v=0.4)
    # net.stopMobility(time=mEnd)
    ...
    for i in range(numOfSta+1, numOfSta+numOfSSta+1):
        sta_name = 'sta'+str(i)
        station = nodes[sta_name]
        station.cmdPrint('ifconfig '+sta_name+'-wlan0 10.0.'+str(i+1)+'.0/32')
        station.cmdPrint('ip route add default via 10.0.'+str(i+1)+'.0 dev '+sta_name+'-wlan0')
    ...
    host.cmdPrint('pushd ~/D-ITG-2.8.1-r1023/bin')
    host.cmdPrint('./ITGRecv &')
    host.cmdPrint('PID=$!')
    host.cmdPrint('popd')
    ...
    host.cmdPrint('tcpdump -i h1-eth0 -w '+folderName+'/h1.pcap &')
